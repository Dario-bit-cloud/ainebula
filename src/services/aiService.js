...
export async function generateResponse(message, modelId = 'nebula-5.1-instant', chatHistory = []) {
  ...
  // Risposta più intelligente se contiene domande specifiche
  let response = responses[modelId] || responses['nebula-5.1-instant'];
  
  if (message.toLowerCase().includes('ciao') || message.toLowerCase().includes('hello')) {
    response = 'Ciao! Come posso aiutarti oggi?';
  } else if (message.toLowerCase().includes('cosa puoi fare')) {
    response = 'Posso aiutarti con molte cose: rispondere a domande, generare codice, scrivere contenuti, analizzare dati e molto altro. Dimmi pure cosa ti serve!';
